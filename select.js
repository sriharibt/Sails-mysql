...
function encrypt(plain_text) {
        ...
        var crypted = c.update(plain_text, 'utf8', 'hex') + c.final('hex');
...
function decrypt(enc_text) {
        ...
        var decrypted = dc.update(enc_text, 'hex', 'utf8') + dc.final('utf8');
        return decrypted;
...
console.log('THIS IS IN SIMPLE SELECT');
  // Escape table name
  ...
  selectKeys.forEach(function(select) {
    ...
    if(select.alias) {
      ...
      query += utils.escapeName(select.table, self.escapeCharacter) + '.' + utils.escapeName(select.key, self.escapeCharacter) + ' AS ' + self.escapeCharacter + select.alias + '___' + select.key + self.escapeCharacter + ', ';
        ...
        if(utils.escapeName(select.key, self.escapeCharacter)=='`regex`')
          {
            query += "aes_decrypt(unhex("+ utils.escapeName(select.table, self.escapeCharacter) + '.' + utils.escapeName(select.key, self.escapeCharacter)+"),'test') as `regex`" + ', ';
          }
        else if(utils.escapeName(select.key, self.escapeCharacter)=='`mapping_json`')
            {
              query += "aes_decrypt(unhex("+ utils.escapeName(select.table, self.escapeCharacter) + '.' + utils.escapeName(select.key, self.escapeCharacter)+"),'test') as `mapping_json`" + ', ';
            }
        else {
...
/*
 ...
 selectKeys.forEach(function(select) {
        ...
        if(select.alias) {
            query +=  decrypt(utils.escapeName(select.table, self.escapeCharacter) + '.' + utils.escapeName(select.key, self.escapeCharacter),'test') + ' AS ' + self.escapeCharacter + select.alias + '___' + select.key + self.escapeCharacter + ', ';

        }

        else {

            query += decrypt(utils.escapeName(select.table, self.escapeCharacter) + '.' + utils.escapeName(select.key, self.escapeCharacter),'test') + ', ';
            }
});
